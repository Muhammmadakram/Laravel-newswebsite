...
<body>
    ...
    <div class="container-xxl position-relative bg-white d-flex p-0">
        ...
        <div class="content">
            ...
            <div style="margin-right: 25px;" class="float-end  py-2 container-fluid">
                <button type="button" class="btn-md float-end mt-3 py-2 btn btn-secondary text-light"
                    ...
                    Add Flash<i style="top:2px; left:5px; font-size:22px; position:relative;"
            ...
            <div class="modal fade" id="exampleModal" tabindex="-1" aria-labelledby="exampleModalLabel"
                ...
                <div class="modal-dialog ">
                    <div class="modal-content">
                        <form id="addnewsflash" method="POST" >
                            @csrf
                            <div class="modal-header ">
                                <h4 class="modal-title text-dark fw-bold" id="exampleModalLabel">Add Flash</h4>
                                <button type="button" class="btn-close" data-bs-dismiss="modal"
                                    aria-label="Close"></button>
                            </div>
                            <div class="modal-body">
                                <div class="form-group">
                                    <label class="fw-bolder text-dark">News Flash</label>
                                    <textarea name="newsflash" placeholder="Enter a news flash"
        ...
        <script type="text/javascript">
            ...
            $(document).ready(function() {
                $('#addnewsflash').submit(function(e) {
                    e.preventDefault();
                    // Get the form data
              ...
              $(document).ready(function() {
                ...
                function getProducts(page) {
                    $.ajax({
                        url: '/pagination/flash?page=' + page,
                        success: function(response) {
         ...
         // delete flash
            $(document).ready(function() {
                $(document).on('click', '#deleteflash', function() {
                    var productId = $(this).data('id');
                    Swal.fire({
                        title: 'Are you sure?',
                        text: 'You will not be able to recover this Flash!',
                        icon: 'warning',
                    ...
                    cancelButtonColor: '#fd5c63',
                        ...
                        if (result.isConfirmed) {
                            $.ajax({
                                url: '/removeflash/' + productId,
                                type: 'DELETE',
                                data: {
                                    "_token": "{{ csrf_token() }}"
                                },
                                success: () => {
                                    Swal.fire('Deleted!', 'The Flash has been deleted.',
                                        'success');
            ...
            $(document).ready(function() {
                $(document).on('click', '#updateflash', function() {
                    let uid = $(this).data('uid');
                    let newflash = $(this).data('flash');
                    // Set the values of the input fields in the update form
                    $('#up_id').val(uid);
                    $('#up_flash').val(newflash);
                });
            });
  ...
  $('#editflash').on('input', function() {
    formChanged = true;
  });

  $('#editflash').submit(function(e) {
    e.preventDefault();
    ...
    if (!formChanged) {
      ...
      return;
                    ...
                    // Get the form data
                    let up_id = $('#up_id').val();
                    let flash = $('#up_flash').val();

                    let formData = new FormData();
                    formData.append('up_id', up_id);
                    formData.append('up_flash', flash);
...
<div class="modal fade " id="updateModal" tabindex="-1" aria-labelledby="updateModalLabel" aria-hidden="true">
    <div class="modal-dialog ">
        <div class="modal-content">
            <form id="editflash" method="POST">
                @csrf
                <input type="hidden" id="up_id">
                <div class="modal-header ">
            <h4 class="modal-title text-dark fw-bold" id="updateModalLabel">Update Flash</h4>
         <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
        </div>
             <div class="modal-body">
           <div class="form-group">
          <label class="fw-bold text-dark">News Flash</label>
              <textarea name="up_flash" id="up_flash" placeholder="Enter a news flash"
              class="form-control text-dark bg-light" required>
